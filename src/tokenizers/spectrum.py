...
class LookUpFreeQuantizer(nn.Module):
    """Look-up-Free Quantizer (LFQ) - simplified version.
    
    Uses straight-through estimator on continuous latents with commitment loss.
    """
    ...
    def forward(self, z):
        ...
        # Straight-through quantization: round to nearest integer in [-1, 1]
        z_q = torch.round(z).clamp(-1, 1)
        ...
        # Compute indices: map [-1, 1] to [0, codebook_size-1]
        indices = ((z_q + 1) / 2 * (self.codebook_size - 1)).long().clamp(0, self.codebook_size - 1)
    ...
    def decode(self, indices):
        ...
        z = indices.float() / (self.codebook_size - 1) * 2 - 1
        return z
